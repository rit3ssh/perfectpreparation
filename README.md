...
- A sample resume PDF (`resume.pdf`) generated using this project is included at the repository root. You can open `resume.pdf` directly from the repo root to view example output. If you'd like to generate your own resume using the project, follow the normal application workflow (create or populate resume data in the app) and export to PDF if that feature is exposed by the UI or backend.
